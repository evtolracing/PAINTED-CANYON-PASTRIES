  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        fulfillmentType: fulfillment.toUpperCase(),
      ...
      clearCart();
      navigate(`/order-confirmation/${data.data?.orderNumber || data.orderNumber || 'success'}`);
    } catch (err) {
      showSnackbar(err.response?.data?.message || 'Failed to place order. Please try again.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
                ...
                <Box>
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                      <TextField label="Name" fullWidth required value={guestInfo.name}
